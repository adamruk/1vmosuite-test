...
- **Phase 2d** — Direct PyQt5 → PySide6 migration (no qtpy, no qt_compat shim). 12 source files migrated; 36/36 py_compile clean; all four apps launch and a CPU `libx264` render produced playable output on Mac Python 3.14 / PySide6 6.9.3. GPU/NVENC path not yet verified (no NVIDIA hardware on Junaid's Mac). Phase A perf-tuning experiment (gpu_enabled auto-on when NVENC detected; preset p4 → p5; concurrency 2 → 3; `-threads 0` on the CPU branch) was REVERTED in the same branch in Phase 1 / `ffcf529`; defaults remain at the pre-Phase-A baseline (`False` / `p4` / `2` / `h264_nvenc` / `retry_cpu` / `rename`) per ADR-0008 line 77 ("Production gpu_preset config default remains 'p4'"). Codex post-completion review landed a preset-count tooling alignment and a *partial* `_reload_config_settings` fix that now refreshes the 5 GPU Pipeline keys live (B-014 remains **Open** for the remaining `num_threads`, `show_ffmpeg_command`, `open_output_when_done`).
