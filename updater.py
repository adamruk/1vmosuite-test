...
from PyQt5.QtWidgets import QDialog, QMessageBox, QProgressDialog, QProgressBar, QVBoxLayout, QHBoxLayout, QWidget, QLabel, QPushButton
...
class UpdaterDialog(QDialog):
    ...
    def init_ui(self):
        ...
        update_msg = QLabel('📥 The tool will download and install the update automatically.\n\nDo you want to update now?')
...
class DriveUpdater:
    def __init__(self):
        ...
        self.version_file = os.path.join(os.path.dirname(__file__), 'assets', 'Version AutoRender.json')
        try:
            icon_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'assets', 'update.ico'))
        ...
        try:
            ...
            for exe_name, base_name, version, download_link in version_info:
                if base_name == app_name:
                    ...
                    success, status_message = self._download_and_install(download_link, f"{app_name} v{saved_version}.{('rar' if is_asset else 'exe')}", exe_name, is_asset)
                    if success:
                        ...
                        delete_msg.setText(f'🗑️ Do you want to delete the old version?\n\n📂 {current_exe_name}')
            ...
            for line in content.split('\n')[1:]:
                if line.strip():
                    ...
                    if len(parts) >= 3:
                        ...
                        version_info.append((exe_name, base_name, version, download_link))
                        print(f'Đã đọc được thông tin: {exe_name} - {version} - {download_link}')
            print(f'Đã đọc được {len(version_info)} phiên bản từ Google Sheet')
            return version_info
        except Exception as e:
            print(f'Lỗi khi đọc Google Sheet: {str(e)}')
    def _download_and_install(self, download_link: str, current_exe: str, new_exe_name: str, is_asset: bool=False) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (True, f'New version has been downloaded ({self._format_size(downloaded)})')
        except Exception as e:
