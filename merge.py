...
_setup_file_logging(SCRIPT_DIR, "video_merge.log")
...
class MergeWorker(QRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            if self.output_format != "Free" and self.layout in [
                "Horizontal",
                "Vertical",
            ]:
            ...
            for i in range(len(self.input_files)):
                ...
                if ext in [".jpg", ".jpeg", ".png", ".bmp", ".gif", ".tiff"]:
                    filter_complex.append(
                        f"[{i}:v]scale={new_widths[i]}:{new_heights[i]}:force_original_aspect_ratio=decrease,pad={new_widths[i]}:{new_heights[i]}:(ow-iw)/2:(oh-ih)/2,setpts=PTS-STARTPTS,trim=duration=5[v{i}]"
                    )
                else:
                    filter_complex.append(
                        f"[{i}:v]scale={new_widths[i]}:{new_heights[i]}:force_original_aspect_ratio=decrease,pad={new_widths[i]}:{new_heights[i]}:(ow-iw)/2:(oh-ih)/2[v{i}]"
                    )
            if len(self.input_files) == 1:
                ...
                if self.layout == "Horizontal":
                    ...
                    if self.layout == "Vertical":
                        ...
                        if self.layout == "2x2 Grid":
                            ...
                            filter_complex.append(
                                f"[v1]format=yuva420p,geq='r=r(X,Y):g=g(X,Y):b=b(X,Y):a=255*{self.opacity / 100}'[fg]"
                            )
            ...
            return_code = core_ffmpeg_runner.run_ffmpeg(
                ...
                on_progress=lambda pct: self.signals.progress_updated.emit(
            ...
            if self.num_videos == 3:
                if self.layout_mode == "Horizontal":
                    ...
                    for i in range(3):
                        ...
                        painter.drawText(
                            x + i * w + w // 2 - 10, y + height // 2, str(i + 1)
                        )
                else:
                    h = height // 3
                    colors = ["#007bff", "#28a745", "#fd7e14"]
                    for i in range(3):
                        ...
                        painter.drawText(
                            x + width // 2 - 10, y + i * h + h // 2, str(i + 1)
                        )
            else:
                if self.num_videos == 4:
                    if self.layout_mode == "Horizontal":
                        ...
                        if self.layout_mode == "Vertical":
                            ...
                            for i in range(4):
                                painter.fillRect(
                                    x, y + i * h, width, h, QColor(colors[i])
                                )
                                painter.drawText(
                                    x + width // 2 - 10, y + i * h + h // 2, str(i + 1)
                                )
...
class VideoMergeTool(QMainWindow):
    def __init__(self):
        ...
        self.USER_DATA_DIR = resolve_or_die(
            SCRIPT_DIR,
            on_error=lambda msg: QMessageBox.critical(None, "1vmo Merge", msg),
        )
    ...
    def create_input_frame(self) -> QFrame:
        ...
        self.btn_group1 = self.create_video_button(
            "Group 1 (0)", self.select_group1_videos, "#e3f2fd", "#1976d2", "#bbdefb"
        )
        self.btn_group2 = self.create_video_button(
            "Group 2 (0)", self.select_group2_videos, "#c8e6c9", "#2e7d32", "#a5d6a7"
        )
        self.btn_group3 = self.create_video_button(
            "Group 3 (0)", self.select_group3_videos, "#ffe0b2", "#f57c00", "#ffb74d"
        )
        self.btn_group4 = self.create_video_button(
            "Group 4 (0)", self.select_group4_videos, "#ffcdd2", "#c62828", "#ef9a9a"
        )
        self.btn_delete = self.create_video_button(
            "Delete",
        ...
        self.tree_videos.setHeaderLabels(
            ["Type", "No.", "Filename", "Duration", "Resolution", "Format"]
        )
    ...
    def update_preview(self):
        ...
        output_format = (
            self.combo_format.currentText() if self.combo_format.isVisible() else "Free"
        )
        if layout_mode == "Overlay":
            self.overlay_frame.setVisible(True)
            self.ratio_frame.setVisible(False)
        ...
        if num_videos == 2:
            self.preview_widget.update_preview(
                layout_mode, num_videos, ratio, output_format
            )
        else:
    ...
    def create_progress_frame(self) -> QFrame:
        ...
        self.thread_labels = []
        for i in range(3):
            row, label, status, progress = (
                core_widgets.create_thread_row_with_status_class(i)
            )
    ...
    def update_video_counts(self):
        ...
        self.btn_delete.setEnabled(
            bool(
                self.group1_videos
                or self.group2_videos
                or self.group3_videos
                or self.group4_videos
                or self.audio_files
            )
        )
    ...
    def select_videos(self, video_list: List[str], title: str, config_key: str):
        try:
            ...
            file_paths = core_file_picker.pick_files(
                self, title, initial_dir, core_file_picker.MEDIA_FILTER
            )
            if file_paths:
    ...
    def select_group1_videos(self):
        self.select_videos(
            self.group1_videos, "Select Group 1 Files", "last_group1_dir"
        )

    def select_group2_videos(self):
        self.select_videos(
            self.group2_videos, "Select Group 2 Files", "last_group2_dir"
        )

    def select_group3_videos(self):
        self.select_videos(
            self.group3_videos, "Select Group 3 Files", "last_group3_dir"
        )

    def select_group4_videos(self):
        self.select_videos(
            self.group4_videos, "Select Group 4 Files", "last_group4_dir"
        )
            ...
            if reply == QMessageBox.Yes:
                for item in selected_items:
                    ...
                    if file_type == "Group 1":
                        self.group1_videos = [
                            v
                            for v in self.group1_videos
                            if os.path.basename(v) != file_path
                        ]
                    else:
                        if file_type == "Group 2":
                            self.group2_videos = [
                                v
                                for v in self.group2_videos
                                if os.path.basename(v) != file_path
                            ]
                        else:
                                ...
                                if file_type == "Group 4":
                                    ...
                                    if file_type == "Audio":
                                        self.audio_files = [
                                            ...
                                            if os.path.basename(a) != file_path
                                        ]
                self.update_video_tree()
                self.update_video_counts()
                self.btn_audio.setText(f"🎵 Audio ({len(self.audio_files)})")
        ...
        try:
            ...
            if ext in [".jpg", ".jpeg", ".png", ".bmp", ".gif", ".tiff"]:
                return (
                    file_path,
                    "Image",
                    self.get_image_resolution(file_path),
                    format,
                )
    ...
    def get_image_resolution(self, image_path: str) -> str:
        try:
            ...
            self.logger.error(
                f"Error getting image resolution for {image_path}: {str(e)}"
            )
            return "Unknown"
    ...
    def update_video_tree(self):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = []
            video_types = [
                (self.group1_videos, "Group 1", "#007bff"),
                (self.group2_videos, "Group 2", "#28a745"),
                (self.group3_videos, "Group 3", "#fd7e14"),
                (self.group4_videos, "Group 4", "#dc3545"),
                (self.audio_files, "Audio", "#7b1fa2"),
            ]
            counters = {
                "Group 1": 1,
                "Group 2": 1,
            ...
            for files, file_type, color in video_types:
                for file in files:
                    futures.append(
                        (
                            executor.submit(self.fetch_video_metadata, file),
                            file_type,
                            color,
                        )
                    )
    ...
    def update_box_color(self, index: int, color: str):
        if 0 <= index < len(self.progress_boxes):
            styles = {
                "green": "background-color: #4CAF50; border: 1px solid #2E7D32;",
                "yellow": "background-color: #FFC107; border: 1px solid #FFA000;",
                "red": "background-color: #F44336; border: 1px solid #D32F2F;",
                "default": "background-color: lightgray; border: 1px solid #666666;",
            }
            self.progress_boxes[index].setStyleSheet(
                f"{styles.get(color, styles['default'])} border-radius: 2px;"
            )
        ...
        if self.is_merging:
            if (
                QMessageBox.question(
                    self,
                    "Confirm",
                    "Merging in progress. Do you want to cancel and start new?",
                    QMessageBox.Yes | QMessageBox.No,
                )
                == QMessageBox.Yes
            ):
        ...
        if self.is_merging:
            ...
            if reply == QMessageBox.Yes:
                ...
                QMessageBox.information(
                    self, "Information", "Merging process has been stopped."
                )
                self.is_merging = False
                self.btn_start.setEnabled(True)
                self.save_config()
        ...
        if self.CONFIG_FILE.exists():
            try:
                with open(self.CONFIG_FILE, "r", encoding="utf-8") as f:
                    ...
                    self.combo_layout.setCurrentText(
                        config.get("layout_mode", "Horizontal")
                    )
                    self.combo_audio.setCurrentText(
                        config.get("audio_source", "Longest")
                    )
                    if hasattr(self, "slider_opacity"):
                        self.slider_opacity.setValue(config.get("opacity", 50))
                        self.opacity_value.setText(str(config.get("opacity", 50)))
                    if hasattr(self, "combo_overlay_group"):
                        self.combo_overlay_group.setCurrentText(
                            config.get("overlay_group", "Group 1")
                        )
        ...
        if self.is_merging:
            if (
                QMessageBox.question(
                    self,
                    "Exit",
                    "Merging in progress. Do you really want to exit?",
                    QMessageBox.Yes | QMessageBox.No,
                )
                == QMessageBox.Yes
            ):
    ...
    def open_output_directory(self):
        if not (self.output_directory and os.path.exists(self.output_directory)):
            ...
            return
        # macOS stabilization (Step 3): try the platform-correct opener
        # first. Previous order tried xdg-open first on every non-
        # Windows platform, which fails-then-falls-through on macOS
        # (every click emits a swallowed FileNotFoundError).
        try:
            if os.name == "nt":
    ...
    def select_audio_files(self):
        ...
        try:
            ...
            file_paths = core_file_picker.pick_files(
                self, "Select Audio Files", initial_dir, core_file_picker.AUDIO_FILTER
            )
            if file_paths:
