                ...
                if mode == "split_by_time":
                    ...
                    command.extend(
                        [
                            ...
                            "mp4",
                            "-force_key_frames",
                            f"expr:gte(t,n_forced*{duration})",
                            temp_output,
                        ]
                    )
                else:
                    if mode == "split_by_parts":
                        ...
                        if mode == "trim_ends":
                            ...
                            command.extend(
                                ["-ss", str(start), "-t", str(duration), temp_output]
                            )
                        else:
                ...
                if return_code == 0:
                    ...
                    if temp_files and all((os.path.getsize(f) > 0 for f in temp_files)):
                        for temp_file in temp_files:
                            ...
                            self.logger.info(
                                f"Segment {temp_file}: Duration = {segment_duration}s"
                            )
                            shutil.move(
                                temp_file,
                                final_output.replace(
                                    "%03d", os.path.basename(temp_file)[(-7):(-4)]
                                ),
                            )
                        ...
                        self.signals.cut_completed.emit(
                            os.path.basename(self.output_path)
                        )
                    else:
                        raise RuntimeError(
                            f"No valid output files created for: {self.output_path}"
                        )
                else:
                    raise RuntimeError(
...
class VideoCutterTool(QMainWindow):
    def __init__(self):
        ...
        self.USER_DATA_DIR = resolve_or_die(
            SCRIPT_DIR,
            on_error=lambda msg: QMessageBox.critical(None, "1vmo Cutter", msg),
        )
    ...
    def create_input_frame(self) -> QFrame:
        ...
        self.btn_videos = self.create_video_button(
            "Videos (0)", self.select_videos, "#e3f2fd", "#1976d2", "#bbdefb"
        )
    ...
    def create_config_frame(self) -> QFrame:
        ...
        self.combo_cut_mode.addItems(
            ["Split by Time", "Split by Parts", "Trim Start/End", "Specific Time Range"]
        )
        ...
        self.start_trim_input.addItems(
        ...
        self.start_range_input.addItems(
            ["0", "1", "2", "5", "10", "15", "20", "30", "60"]
        )
    ...
    def delete_selected_videos(self):
        ...
        if not selected_items:
            ...
            for item in selected_items:
                ...
                self.video_list[:] = [
                    v for v in self.video_list if os.path.basename(v) != filename
                ]
            self.update_video_tree()
    ...
    def fetch_video_metadata(self, video_path: str) -> Tuple[str, str, str]:
        if video_path in self.video_metadata_cache:
            ...
            try:
                ...
                self.video_metadata_cache[video_path] = (
                    video_path,
                    duration,
                    resolution,
                )
    ...
    def update_box_color(self, index: int, color: str):
        ...
        if 0 <= index < len(self.progress_boxes):
            styles = {
                "green": "background-color: #28a745; border: 1px solid #218838;",
                "yellow": "background-color: #ffc107; border: 1px solid #d39e00;",
                "red": "background-color: #dc3545; border: 1px solid #bd2130;",
                "default": "background-color: #f0f0f0; border: 1px solid #dee2e6;",
            }
            self.progress_boxes[index].setStyleSheet(
                f"{styles.get(color, styles['default'])} border-radius: 4px;"
            )
        ...
        if self.is_processing:
            if (
                QMessageBox.question(
                    self,
                    "Confirm",
                    "Cutting in progress. Do you want to cancel and start new?",
    ...
    def validate_inputs(self) -> bool:
        ...
        try:
            if mode == "Split by Time":
                ...
                if duration <= 0:
                    QMessageBox.warning(
                        self, "Warning", "Segment duration must be greater than 0."
                    )
                    return False
            elif mode == "Split by Parts":
                parts = int(self.parts_input.currentText())
                if parts < 1:
                    QMessageBox.warning(
                        self, "Warning", "Number of parts must be at least 1."
                    )
                    return False
            elif mode == "Trim Start/End":
                start = float(self.start_trim_input.currentText())
                ...
                if start == 0 and end == 0:
                    QMessageBox.warning(
                        self, "Warning", "Please specify at least one trim value."
                    )
                    return False
            elif mode == "Specific Time Range":
                start = float(self.start_range_input.currentText())
                ...
                if start >= end:
                    QMessageBox.warning(
                        self, "Warning", "End time must be greater than start time."
                    )
                    return False
            return True
        except ValueError:
        ...
        if self.is_processing:
            if (
                QMessageBox.question(
                    self,
                    "Exit",
                    "Cutting in progress. Do you really want to exit?",
                    QMessageBox.Yes | QMessageBox.No,
                )
                == QMessageBox.Yes
            ):
    ...
    def open_output_directory(self):
        ...
        # first instead of swallowing a FileNotFoundError. The previous
        # order (xdg-open → open) emitted a spurious error on macOS
        # every time. Order now: Windows (startfile), macOS (open),
        # Linux/other (xdg-open with `open` as final fallback).
        try:
            if os.name == "nt":
