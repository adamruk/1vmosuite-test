...
class MergeCoordinator(QObject):
    per_video_started = Signal(int, str, str)
    finished = Signal()
    error_occurred = Signal(str)
...
class VideoMergerTool(QMainWindow):
    ...
    def create_input_frame(self) -> QFrame:
        ...
        self.btn_main = self.create_video_button(
            "Main Videos (0)", self.select_main_videos, "#e3f2fd", "#1976d2", "#bbdefb"
        )
        self.btn_intro = self.create_video_button(
            "Intro Videos (0)",
            self.select_intro_videos,
            "#c8e6c9",
            "#2e7d32",
            "#a5d6a7",
        )
        self.btn_outro = self.create_video_button(
            "Outro Videos (0)",
            self.select_outro_videos,
            "#ffe0b2",
            "#f57c00",
            "#ffb74d",
        )
        ...
        self.tree_videos.setHeaderLabels(
            ["Type", "No.", "Filename", "Duration", "Resolution"]
        )
    ...
    def create_config_frame(self) -> QFrame:
        ...
        configs = [
            ("Main Mix Mode", "combo_merge_mode_main", ["Random", "Sequential"], 200),
            ("Intro Mix Mode", "combo_merge_mode_intro", ["Random", "Sequential"], 200),
            ("Outro Mix Mode", "combo_merge_mode_outro", ["Random", "Sequential"], 200),
            ("Main Videos to Mix", "combo_num_merge", [], 100, True),
            ("Total Output Videos", "combo_num_output", [], 100, True),
            (
                "Output Name Option",
                "combo_output_naming",
                ["Time + Index", "First Video Name + Time + Index"],
                400,
            ),
        ]
            ...
            existing.update(
                {
                    ...
                    "last_intro_video_dir": os.path.dirname(self.intro_videos[0])
                    if self.intro_videos
                    else "",
                    "last_outro_video_dir": os.path.dirname(self.outro_videos[0])
                    if self.outro_videos
                    else "",
    ...
    def select_videos(self, video_list: List[str], title: str, config_key: str):
        ...
        try:
            ...
            file_paths = core_file_picker.pick_files(
                self, title, initial_dir, core_file_picker.VIDEO_FILTER
            )
            if file_paths:
    ...
    def select_intro_videos(self):
        self.select_videos(
            self.intro_videos, "Select Intro Videos", "last_intro_video_dir"
        )

    def select_outro_videos(self):
        self.select_videos(
            self.outro_videos, "Select Outro Videos", "last_outro_video_dir"
        )
    ...
    def delete_selected_videos(self):
        ...
        if not selected_items:
            ...
            for item in selected_items:
                ...
                video_list = {
                    "Main": self.video_list,
                    "Intro": self.intro_videos,
                    "Outro": self.outro_videos,
                }.get(video_type)
                if video_list:
                    video_list[:] = [
                        v for v in video_list if os.path.basename(v) != filename
                    ]
            self.update_video_tree()
            self.btn_delete.setEnabled(
                bool(self.video_list or self.intro_videos or self.outro_videos)
            )
            self.update_video_counts()
    ...
    def fetch_video_metadata(self, video_path: str) -> Tuple[str, str, str]:
        ...
        try:
            return (
                ...
                self.get_video_resolution(video_path),
            )
        except Exception as e:
    ...
    def update_video_tree(self):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = []
            video_types = [
                (self.video_list, "Main", "#007bff"),
                (self.intro_videos, "Intro", "#28a745"),
                (self.outro_videos, "Outro", "#fd7e14"),
            ]
            ...
            for videos, video_type, color in video_types:
                for video in videos:
                    futures.append(
                        (
                            executor.submit(self.fetch_video_metadata, video),
                            video_type,
                            color,
                        )
                    )
        ...
        self.combo_num_output.addItems(
    ...
    def validate_inputs(self) -> bool:
        ...
        if not self.video_list:
            QMessageBox.warning(
                self, "Warning", "Please select at least one main video."
            )
            return False
        ...
        try:
            ...
            if num_output < 1:
                QMessageBox.warning(
                    self, "Warning", "Total output videos must be at least 1."
                )
                return False
            if self.intro_videos and (
                not all((os.path.exists(v) for v in self.intro_videos))
            ):
                QMessageBox.warning(self, "Warning", "Some intro videos are missing.")
                return False
            if self.outro_videos and (
                not all((os.path.exists(v) for v in self.outro_videos))
            ):
    ...
    def on_merge_completed(self, output_filename: str):
        ...
        self.progress_label.setText(
            f"Progress: {self.processed_output}/{self.total_output}"
        )
    ...
    def load_last_paths(self):
        ...
        config = self.config
        self.combo_merge_mode_main.setCurrentText(
            config.get("merge_mode_main", "Random")
        )
        self.combo_merge_mode_intro.setCurrentText(
            config.get("merge_mode_intro", "Random")
        )
        self.combo_merge_mode_outro.setCurrentText(
            config.get("merge_mode_outro", "Random")
        )
        ...
        for video_list, attr in [
            (config.get("last_videos", []), "video_list"),
            (config.get("last_intro_videos", []), "intro_videos"),
            (config.get("last_outro_videos", []), "outro_videos"),
        ]:
    ...
    def open_output_directory(self):
        ...
        # first instead of always trying xdg-open and falling through.
        try:
            if os.name == "nt":
