...
- `core.preset_loader.derive_slug` + `derive_ids_for_presets`: 5-step slug transform (NFKD normalize, strip combining marks, lowercase, replace non-alphanumeric runs with hyphen, strip trailing hyphens) and ID derivation from (group, name) tuples with conditional `-N` suffix on collision. Per ADR-0006. [TBD] [v2c-c-4]
- `core.preset_loader.Preset.id`: new required first field on the dataclass; load-bearing identity replacing position-based composite (group, name). Per ADR-0006. [TBD] [v2c-c-4]
- `core.encoder_schema.EncoderPreset.id` + `ID_PATTERN` regex enforcing `builtin:[group/]name[-N]` and `user:name[-N]` (user namespace is flat, never group-prefixed). Library-level `_ids_unique` model_validator enforces uniqueness. [TBD] [v2c-c-4]
- `docs/decisions/ADR-0006-preset-id-schema-v2.md`: identity-model decision record covering schema_version=2 bump, ID_PATTERN, slug rule, prefix syntax, conditional suffix, lazy v1→v2 migration semantics, EncoderDialog UI-Min, _builtin_preset_count removal. [TBD] [v2c-c-4]
- `tools/check_preset_ids.py`: validator confirming all 111 ids in `assets/Encoder.json` match ID_PATTERN, are unique, are builtin: namespace, and the 10-entry layer-overlay collision cluster gets correctly suffixed -1..-10. ADR-0001 aligned. [TBD] [v2c-c-4]
- `tools/test_id_migration.py`: tempdir-based smoke validator for v1→v2 lazy migration (load v1 derives ids, save persists v2, v2 round-trips). [TBD] [v2c-c-4]
- `tests/smoke-2c-c-4-ids-20260427.log`: preset-id validation smoke output (4/4 checks PASS for 111-preset library). [TBD] [v2c-c-4]
- `tests/smoke-2c-c-4-migration-20260427.log`: v1→v2 migration smoke output (3/3 PASS). [TBD] [v2c-c-4]
