...
- Pre-commit toolchain installed (pre-commit, commitizen, ruff, markdownlint, custom ADR validator) [CHANGELOG-hash]
- scripts/adr_lint.py — validates ADR header fields, status values, date consistency, filename pattern [CHANGELOG-hash]
- .pre-commit-config.yaml, .markdownlint.yaml, .cz.toml (commitizen config; pyproject.toml decision deferred) [CHANGELOG-hash]
